package controller
{
	...
	public class StringUtils
	{
		...
		public static function addVariableParams(text:String , params:Array,delimier:String):String{
			for(var count:int = 0 ; count < params.length ; count++){
				var pattern:RegExp = new RegExp(delimier+(count+1),"gi")
				text = text.replace(pattern,params[count]);
			}
			return text
		}
		
		/**
		 *returns the name component from a file name.  (all the other text from the <filename> minus the last string after the last <delimiter> occurrence)
		 * e.g.
		 ...
		 * @param delimiter the delimiter to use, "."(dot) by default
		 * @return the name component of the file 
		 * 
		 */
		public static function getFileNameMinusExtension(filename:String, delimiter:String="."):String
		{
			var newFilename:String = "";
			var elements:Array = filename.split(delimiter);
			...
			{
				if(index < (len - 1)){
					newFilename += elements[index] + delimiter;
				}else{
					newFilename += elements[index];
				}
			}
			return newFilename;
		}
		
		public static function encode(encodeString:String):String{
			
			var output:String = "";
			
			for(var count:int = 0 ; count < encodeString.length ; count++){
				var eachCharCode:int = encodeString.charCodeAt(count);
				output = output + eachCharCode + "E";
			}
			return output 
		}
		
		public static function decode(stringToDecode:String):String{
			var output:String = "";
			var tempData:String = "";
			
			for(var count:int = 0  ; count < stringToDecode.length ; count++){
				var eachCharCode:String = stringToDecode.charAt(count);
				if(eachCharCode == "E"){
					output  = output + String.fromCharCode(int(tempData));
					tempData = "";
				}else{
					tempData = tempData + eachCharCode;
				}
			}
			return output;
		}
		...
		public static function removeAllInBetween(p_string:String, p_start:String, p_end:String):String {
			var stringAfterRemoving:String = removeFirstInBetween(p_string,p_start,p_end);
			if(stringAfterRemoving == p_string){
				return stringAfterRemoving;
			}else{
				return removeAllInBetween(stringAfterRemoving,p_start,p_end);
			}
			return "";
		}
		
		public static function removeFirstInBetween(p_string:String, p_start:String, p_end:String):String
		{
			var betweenString:String = between(p_string,p_start,p_end);
			
			return remove(p_string,p_start + betweenString + p_end);
		}
		...
		/**
		 * Replace an array of search terms from an input string
		 * 
		 * @param patternArray
		 * @param inp
		 * @param repl
		 ...
		 * 
		 */		
		public static function replaceArrayOfString(patternArray:Array,inp:String,repl:String):String
		{
			for each (var p:String in patternArray) 
			{
				inp = inp.split(p).join(repl);
			}
			return inp;
		}
		...
		public static function encodeToBase64(str:String):String
		{
		...
		public static function clampString(inp:String,range:int = 2):String
		{
			if(isEmpty(inp))
				return "";
			
			if(range > 0 && inp.length > range)
			{
				inp = inp.slice(0,range-2) + "..";
			}
			return inp;
		}
		
		/*public static function decodeFromBase64(str:String):String
		{
			var base64Decoder:Base64Decoder = new Base64Decoder();
			base64Decoder.decode(str);
